...
Bun supports V8-compatible CPU profiling with `--cpu-prof`:
...
Termcast includes a built-in React component profiler that captures render timing for every component. It uses React 19.2's `PerformanceMeasure` API to record what rendered, how long it took, and which file it lives in. Use this when you know the slowdown is in React renders and want to find which specific components are expensive.
