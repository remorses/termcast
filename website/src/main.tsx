/**
 * Spiceflow app entry point for the termcast website.
 * Defines the root layout, landing page, install script endpoints,
 * and OAuth proxy routes.
 */
...
export const app = new Spiceflow()
  ...
  .page('/', async () => {
    return (
      <>
        <Head>
          <Head.Title>{SITE_TITLE}</Head.Title>
          <Head.Meta name='description' content={SITE_DESCRIPTION} />
          <Head.Meta property='og:type' content='website' />
          <Head.Meta property='og:url' content='https://termcast.app' />
          <Head.Meta property='og:title' content={SITE_TITLE} />
          <Head.Meta property='og:description' content={SITE_DESCRIPTION} />
          ...
          <Head.Meta name='twitter:title' content={SITE_TITLE} />
          <Head.Meta name='twitter:description' content={SITE_DESCRIPTION} />
  ...
  // ── Defer example ──
  .page('/defer-example', async () => {
    const projectPromise = Promise.resolve().then(async () => {
      await new Promise((r) => {
        globalThis.setTimeout(r, 1000)
      })
      return 'hi'
    })
    return (
      <Suspense fallback={<p>Loading...</p>}>
        <DeferContent projectPromise={projectPromise} />
      </Suspense>
    )
  })
  ...
  // ── OAuth callback fallback page (shown if redirect doesn't happen) ──
  .page('/oauth/:provider/callback', async () => {
    return (
...
void app.listen(Number(process.env.PORT || 8041))
...
async function DeferContent({
  projectPromise,
}: {
  projectPromise: Promise<string>
}) {
  const location = await projectPromise
  return (
    <main>
...
const TECH_LOGOS: Array<{ label: string; path: string }> = [
  {
    label: 'React',
    path: 'M14.23 12.004a2.236 2.236 0 0 1-2.235 2.236 2.236 2.236 0 0 1-2.236-2.236 2.236 2.236 0 0 1 2.235-2.236 2.236 2.236 0 0 1 2.236 2.236zm2.648-10.69c-1.346 0-3.107.96-4.888 2.622-1.78-1.653-3.542-2.602-4.887-2.602-.41 0-.783.093-1.106.278-1.375.793-1.683 3.264-.973 6.365C1.98 8.917 0 10.42 0 12.004c0 1.59 1.99 3.097 5.043 4.03-.704 3.113-.39 5.588.988 6.38.32.187.69.275 1.102.275 1.345 0 3.107-.96 4.888-2.624 1.78 1.654 3.542 2.603 4.887 2.603.41 0 .783-.09 1.106-.275 1.374-.792 1.683-3.263.973-6.365C22.02 15.096 24 13.59 24 12.004c0-1.59-1.99-3.097-5.043-4.032.704-3.11.39-5.587-.988-6.38-.318-.184-.688-.277-1.092-.278zm-.005 1.09v.006c.225 0 .406.044.558.127.666.382.955 1.835.73 3.704-.054.46-.142.945-.25 1.44-.96-.236-2.006-.417-3.107-.534-.66-.905-1.345-1.727-2.035-2.447 1.592-1.48 3.087-2.292 4.105-2.295zm-9.77.02c1.012 0 2.514.808 4.11 2.28-.686.72-1.37 1.537-2.02 2.442-1.107.117-2.154.298-3.113.538-.112-.49-.195-.964-.254-1.42-.23-1.868.054-3.32.714-3.707.19-.09.4-.127.563-.132zm4.882 3.05c.455.468.91.992 1.36 1.564-.44-.02-.89-.034-1.345-.034-.46 0-.915.01-1.36.034.44-.572.895-1.096 1.345-1.565zM12 8.1c.74 0 1.477.034 2.202.093.406.582.802 1.203 1.183 1.86.372.64.71 1.29 1.018 1.946-.308.655-.646 1.31-1.013 1.95-.38.66-.773 1.288-1.18 1.87-.728.063-1.466.098-2.21.098-.74 0-1.477-.035-2.202-.093-.406-.582-.802-1.204-1.183-1.86-.372-.64-.71-1.29-1.018-1.946.303-.657.646-1.313 1.013-1.954.38-.66.773-1.286 1.18-1.868.728-.064 1.466-.098 2.21-.098zm-3.635.254c-.24.377-.48.763-.704 1.16-.225.39-.435.782-.635 1.174-.265-.656-.49-1.31-.676-1.947.64-.15 1.315-.283 2.015-.386zm7.26 0c.695.103 1.365.23 2.006.387-.18.632-.405 1.282-.66 1.933-.2-.39-.41-.783-.64-1.174-.225-.392-.465-.774-.705-1.146zm3.063.675c.484.15.944.317 1.375.498 1.732.74 2.852 1.708 2.852 2.476-.005.768-1.125 1.74-2.857 2.475-.42.18-.88.342-1.355.493-.28-.958-.646-1.956-1.1-2.98.45-1.017.81-2.01 1.085-2.964zm-13.395.004c.278.96.645 1.957 1.1 2.98-.45 1.017-.812 2.01-1.086 2.964-.484-.15-.944-.317-1.375-.498-1.732-.74-2.852-1.708-2.852-2.476.005-.768 1.125-1.74 2.857-2.475.42-.18.88-.342 1.355-.493zm11.7 3.137c.185.508.352 1.023.505 1.54-.152.52-.32 1.035-.505 1.545-.497.117-1.006.22-1.53.306a35.5 35.5 0 0 0 .53-1.855c.2-.63.38-1.26.53-1.885a23 23 0 0 0 .47.35zm-10.005.003a25 25 0 0 0 .47-.347c.15.625.33 1.254.53 1.884.2.63.38 1.26.53 1.855a25 25 0 0 1-1.53-.306 25 25 0 0 1-.505-1.54c.152-.52.32-1.035.505-1.545zm4.96.605c-.345.608-.713 1.196-1.1 1.76.375.02.753.033 1.14.033s.77-.01 1.14-.033a32 32 0 0 1-1.1-1.76c-.004 0-.076 0-.08 0zm-2.09.076c-.2.333-.397.663-.587.985.14.01.28.02.42.03-.325-.31-.645-.66-.95-1.055a1 1 0 0 0 .117.04zm4.18 0c.305.395.625.745.95 1.055-.14-.01-.28-.02-.42-.03.19-.322.387-.652.587-.985a1 1 0 0 1-.117-.04zm-6.145 1.44c.64.15 1.315.283 2.015.386-.24-.377-.48-.763-.704-1.16-.225-.39-.435-.782-.635-1.174.265.656.49 1.31.676 1.947h-.352zm8.11 0h-.353c.186-.638.41-1.29.676-1.947-.2.392-.41.785-.635 1.174-.224.397-.464.783-.704 1.16.7-.103 1.375-.236 2.015-.386zm-6.62 1.168c.686.72 1.37 1.537 2.02 2.442 1.107-.117 2.154-.298 3.113-.538.112.49.195.964.254 1.42.23 1.868-.054 3.32-.714 3.707-.19.09-.4.127-.563.132-1.012 0-2.514-.808-4.11-2.28.686-.72 1.37-1.537 2.02-2.442-1.107.117-2.154.298-3.113.538-.112-.49-.195-.964-.254-1.42-.23-1.868.054-3.32.714-3.707.19-.09.4-.127.563-.132 1.012 0 2.514.808 4.11 2.28z',
  },
  {
    label: 'Bun',
    path: 'M12 22.596c6.628 0 12-4.338 12-9.688 0-3.318-2.057-6.248-5.219-7.986-1.286-.715-2.297-1.357-3.139-1.89C14.058 2.025 13.08 1.404 12 1.404c-1.097 0-2.334.785-3.966 1.821a49.92 49.92 0 0 1-2.816 1.697C2.057 6.66 0 9.59 0 12.908c0 5.35 5.372 9.687 12 9.687v.001Z',
  },
  {
    label: 'TypeScript',
      ...
      <div className='overflow-hidden relative'>
        ...
        >
          {[...TECH_LOGOS, ...TECH_LOGOS].map((logo, i) => (
            <div
              key={i}
              className='flex-shrink-0 flex items-center justify-center w-[160px] h-[48px] px-2 opacity-70'
            >
      ...
      <div className='max-w-5xl mx-auto flex flex-col gap-16 sm:gap-40'>
        <ProductCard
          ...
          visual={
            <ShowcaseImage
              name='showcase-list'
              alt='Termcast list with sections, search, and graph detail panel'
            />
          }
        />
        <ProductCard
          title='forms &middot; inputs'
          ...
          visual={
            <ShowcaseImage
              name='showcase-form'
              alt='Termcast form with text fields, password, checkboxes, and dropdowns'
            />
          }
          reversed
        />
        <ProductCard
          title='graphs &middot; charts'
          ...
          visual={
            <ShowcaseImage
              name='showcase-charts'
              alt='Termcast bar chart and line graph with colored segments and metadata'
            />
          }
        />
        <ProductCard
          title='heatmaps &middot; data'
          ...
          visual={
            <ShowcaseImage
              ...
              alt='Termcast calendar heatmap with purple color gradients and month labels'
            />
          }
          reversed
        />
...
function CodeLine({ line }: { line: string }) {
  ...
  const tokenPatterns: Array<{ type: string; re: RegExp }> = [
    ...
    {
      type: 'attr',
      re: /^(?:title|subtitle|accessories|actions|tag|value|color|onAction)(?==)/,
    },
    { type: 'keyword', re: /^=>/ },
    { type: 'punct', re: /^[{}()[\];,>=/]/ },
    { type: 'plain', re: /^[^\s"'`<{}()[\];,>=/]+/ },
  ...
  return (
    <>
      {tokens.map((tok, i) => (
        <span key={i} className={colorMap[tok.type] || colorMap.plain}>
          {tok.text}
        </span>
      ))}
    </>
      ...
      <div className='max-w-4xl mx-auto'>
        <div className='text-center mb-10'>
          ...
          <p className='bu-font-sans text-sm sm:text-base text-zinc-500 mt-4 max-w-xl mx-auto'>
            ...
            Import from{' '}
            <code className='bu-font-mono text-zinc-400'>termcast</code>{' '}
            instead of{' '}
            <code className='bu-font-mono text-zinc-400'>@raycast/api</code>.
          </p>
        </div>
        <div className='bg-zinc-950 border border-zinc-800 overflow-hidden'>
          <div className='flex items-center gap-2 px-4 py-2.5 border-b border-zinc-800'>
            ...
            <span className='bu-font-mono text-[11px] text-zinc-600 ml-2'>
              deploy-tool.tsx
            </span>
          </div>
            ...
            <code className='bu-font-mono text-[10px] sm:text-sm leading-relaxed'>
              {code.split('\n').map((line, i) => (
                <div key={i} className='flex'>
                  <span className='text-zinc-700 select-none w-8 text-right mr-4 flex-shrink-0'>
                    {i + 1}
                  </span>
                  <span>
                    <CodeLine line={line} />
        ...
        <div className='bu-font-mono text-sm text-zinc-500 mt-6 max-w-lg'>
          <div className='mb-4'>
            <span className='text-zinc-600'>
              New project or existing Raycast extension -- up and running in
              seconds:
            </span>
          </div>
          <div className='bg-zinc-950 border border-zinc-800 px-5 py-3 text-left'>
            <div>
              ...
              <span className='text-zinc-400'>termcast new my-tool</span>
            </div>
            <div>
              <span className='text-pumpkin-500'>$</span>{' '}
...
const FOOTER_COLUMNS = [
  {
    ...
    links: [
      ...
      {
        label: 'Action Panel',
        href: `${RAYCAST_DOCS}/user-interface/action-panel`,
      },
    ],
  },
  {
...
function Footer() {
  return (
    <footer
      id='footer'
      className='w-full max-w-5xl mx-auto px-6 pt-16 sm:pt-32 pb-12'
    >
      <div className='border-t border-zinc-800 pt-12'>
        ...
        <div className='flex items-center justify-between mt-16 pt-8 border-t border-zinc-800'>
          <span className='bu-font-mono text-xs text-zinc-600'>termcast</span>
        </div>
      </div>
    </footer>
  )
...
function LandingPage() {
  return (
    <div
      className='bg-page-bg min-h-screen'
      style={{
        fontFamily:
