...
export async function copyToClipboard(
    ...
    try {
        if (platform === 'darwin') {
            ...
            logger.log(
                `📋 Copied to clipboard: ${concealed ? '[CONCEALED]' : text}`,
            )
...
export async function openInBrowser(url: string): Promise<void> {
    try {
        if (platform === 'darwin') {
            ...
            logger.log(
                `🌐 Opening browser not supported on ${platform}: ${url}`,
            )
        }
    } catch (error) {
...
export async function openFile(
    ...
    try {
        if (platform === 'darwin') {
            ...
            logger.log(
                `📂 Opening files not supported on ${platform}: ${target}`,
            )
        }
    } catch (error) {
    ...
    try {
        if (platform === 'darwin') {
            ...
            try {
                ...
                await execAsync(
                    `mkdir -p "${trashDir}" && mv "${path}" "${trashDir}/"`,
                )
            }
            logger.log(`🗑️ Moved to trash: ${path}`)
        } else if (platform === 'win32') {
