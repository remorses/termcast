...
export namespace OAuth {
    ...
    export interface AuthorizationRequest
        extends AuthorizationRequestURLParams {
        toURL(): string
    }
    ...
    export class PKCEClient {
            ...
            const request: AuthorizationRequest = {
                ...
                toURL: () => {
                    ...
                    const params = new URLSearchParams({
                        response_type:
                            options.extraParameters?.response_type ||
                            'id_token token',
        ...
        async authorizeDevice(
            ...
            try {
                ...
                logger.log(
                    'Browser opened successfully with device authorization URL',
                )
            } catch (error) {
        ...
        private async pollForToken(
            ...
            const tokenEndpoint = options.endpoint.replace(
                '/device/code',
                '/token',
            )
            ...
            while (Date.now() < expiresAt) {
                ...
                try {
                    ...
                    if (data.error === 'authorization_pending') {
                        ...
                        // Increase interval
                        options.interval =
                            (data.interval || options.interval) + 1
                        logger.log(
                            'Slowing down polling interval to',
                            options.interval,
                        )
        ...
        async authorize(
            ...
            `)
                        ...
                        requestUrl.pathname === '/oauth/implicit-callback'
                    ) {
                        ...
                        req.on('end', () => {
                            try {
                                ...
                                this.setTokens(tokens)
                                    .then(() => {
                                        logger.log(
                                            'Implicit flow tokens saved directly',
                                        )

                                        // Return the tokens (authorizationCode is a dummy value for compatibility)
                                        resolve({
                                            authorizationCode:
                                                'implicit_flow_dummy_code', // Dummy code for compatibility
                                            accessToken: data.access_token,
                                            idToken: data.id_token,
                                            state: data.state,
                                        })
                                    ...
                                    .catch((err) => {
                                        logger.error(
                                            'Failed to save implicit flow tokens:',
                                            err,
                                        )
                                        reject(err)
                                    })
                ...
                server.listen(port, async () => {
                    ...
                    try {
                        ...
                        await execAsync(command)
                        logger.log(
                            'Browser opened for implicit flow authorization',
                        )
                    } catch (error) {
                        server.close()
        ...
        async setTokens(
            ...
            if (stored) {
                ...
                logger.error(
                    'Failed to verify token storage for',
                    this.providerId,
                )
            }
        }
