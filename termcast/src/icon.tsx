import { Icon as RaycastIcon } from '@raycast/api'

// Map of Raycast icons to emojis
export const iconToEmoji: Record<string, string> = {
  'add-person-16': '👤',
  'airplane-16': '✈️',
  'airplane-filled-16': '✈️',
  'airplane-landing-16': '🛬',
  'airplane-takeoff-16': '🛫',
  'airpods-16': '🎧',
  'alarm-16': '⏰',
  'alarm-ringing-16': '⏰',
  'align-centre-16': '📄',
  'align-left-16': '📄',
  'align-right-16': '📄',
  'american-football-16': '🏈',
  'anchor-16': '⚓',
  'app-window-16': '🪟',
  'app-window-grid-2x2-16': '🪟',
  'app-window-grid-3x3-16': '🪟',
  'app-window-list-16': '📋',
  'app-window-sidebar-left-16': '🪟',
  'arrow-clockwise-16': '🔄',
  'arrow-counter-clockwise-16': '🔄',
  'arrow-down-16': '⬇️',
  'arrow-down-circle-16': '⬇️',
  'arrow-down-circle-filled-16': '⬇️',
  'arrow-left-16': '⬅️',
  'arrow-left-circle-16': '⬅️',
  'arrow-left-circle-filled-16': '⬅️',
  'arrow-ne-16': '↗️',
  'arrow-right-16': '➡️',
  'arrow-right-circle-16': '➡️',
  'arrow-right-circle-filled-16': '➡️',
  'arrow-up-16': '⬆️',
  'arrow-up-circle-16': '⬆️',
  'arrow-up-circle-filled-16': '⬆️',
  'arrows-contract-16': '↔️',
  'arrows-expand-16': '↔️',
  'at-symbol-16': '@',
  'band-aid-16': '🩹',
  'bank-note-16': '💵',
  'bar-chart-16': '📊',
  'bar-code-16': '📊',
  'bath-tub-16': '🛁',
  'battery-16': '🔋',
  'battery-charging-16': '🔋',
  'battery-disabled-16': '🪫',
  'bell-16': '🔔',
  'bell-disabled-16': '🔕',
  'bike-16': '🚴',
  'binoculars-16': '🔭',
  'bird-16': '🐦',
  'building-16': '🏢',
  'bullet-points-16': '•',
  'bulls-eye-16': '🎯',
  'bulls-eye-missed-16': '❌',
  'buoy-16': '🛟',
  'calculator-16': '🧮',
  'calendar-16': '📅',
  'camera-16': '📷',
  'car-16': '🚗',
  'cart-16': '🛒',
  'cd-16': '💿',
  'center-16': '⊙',
  'check-16': '✓',
  'check-circle-16': '✅',
  'check-list-16': '☑️',
  'check-rosette-16': '🏆',
  'checkmark-16': '✓',
  'chess-piece-16': '♟️',
  'chevron-down-16': '⌄',
  'chevron-down-small-16': '⌄',
  'chevron-left-16': '‹',
  'chevron-left-small-16': '‹',
  'chevron-right-16': '›',
  'chevron-right-small-16': '›',
  'chevron-up-16': '⌃',
  'chevron-up-down-16': '⌃⌄',
  'chevron-up-small-16': '⌃',
  'circle-16': '○',
  'circle-disabled-16': '⊝',
  'circle-ellipsis-16': '⋯',
  'circle-filled-16': '●',
  'circle-progress-16': '◐',
  'circle-progress-100-16': '●',
  'circle-progress-25-16': '◔',
  'circle-progress-50-16': '◑',
  'circle-progress-75-16': '◕',
  'clear-formatting-16': '🧹',
  'copy-clipboard-16': '📋',
  'clock-16': '🕒',
  'cloud-16': '☁️',
  'cloud-lightning-16': '⛈️',
  'cloud-rain-16': '🌧️',
  'cloud-snow-16': '🌨️',
  'cloud-sun-16': '⛅',
  'code-16': '💻',
  'code-block-16': '📝',
  'cog-16': '⚙️',
  'coin-16': '🪙',
  'coins-16': '💰',
  'command-symbol-16': '⌘',
  
  // Numbers 30-79
  'number-30-16': '30',
  'number-31-16': '31',
  'number-32-16': '32',
  'number-33-16': '33',
  'number-34-16': '34',
  'number-35-16': '35',
  'number-36-16': '36',
  'number-37-16': '37',
  'number-38-16': '38',
  'number-39-16': '39',
  'number-40-16': '40',
  'number-41-16': '41',
  'number-42-16': '42',
  'number-43-16': '43',
  'number-44-16': '44',
  'number-45-16': '45',
  'number-46-16': '46',
  'number-47-16': '47',
  'number-48-16': '48',
  'number-49-16': '49',
  'number-50-16': '50',
  'number-51-16': '51',
  'number-52-16': '52',
  'number-53-16': '53',
  'number-54-16': '54',
  'number-55-16': '55',
  'number-56-16': '56',
  'number-57-16': '57',
  'number-58-16': '58',
  'number-59-16': '59',
  'number-60-16': '60',
  'number-61-16': '61',
  'number-62-16': '62',
  'number-63-16': '63',
  'number-64-16': '64',
  'number-65-16': '65',
  'number-66-16': '66',
  'number-67-16': '67',
  'number-68-16': '68',
  'number-69-16': '69',
  'number-70-16': '70',
  'number-71-16': '71',
  'number-72-16': '72',
  'number-73-16': '73',
  'number-74-16': '74',
  'number-75-16': '75',
  'number-76-16': '76',
  'number-77-16': '77',
  'number-78-16': '78',
  'number-79-16': '79',
  
  'number-80-16': '80',
  'number-81-16': '81',
  'number-82-16': '82',
  'number-83-16': '83',
  'number-84-16': '84',
  'number-85-16': '85',
  'number-86-16': '86',
  'number-87-16': '87',
  'number-88-16': '88',
  'number-89-16': '89',
  'number-90-16': '90',
  'number-91-16': '91',
  'number-92-16': '92',
  'number-93-16': '93',
  'number-94-16': '94',
  'number-95-16': '95',
  'number-96-16': '96',
  'number-97-16': '97',
  'number-98-16': '98',
  'number-99-16': '99',
  'number-list-16': '🔢',
  'paperclip-16': '📎',
  'paragraph-16': '¶',
  'patch-16': '🩹',
  'pause-16': '⏸️',
  'pause-filled-16': '⏸️',
  'pencil-16': '✏️',
  'person-16': '👤',
  'person-circle-16': '👤',
  'person-lines-16': '📇',
  'phone-16': '📞',
  'phone-ringing-16': '📳',
  'pie-chart-16': '📊',
  'pill-16': '💊',
  'pin-16': '📌',
  'pin-disabled-16': '📌',
  'play-16': '▶️',
  'play-filled-16': '▶️',
  'plug-16': '🔌',
  'plus-16': '➕',
  'plus-circle-16': '➕',
  'plus-circle-filled-16': '➕',
  'plus-minus-divide-multiply-16': '➕',
  'plus-square-16': '➕',
  'plus-top-right-square-16': '➕',
  'power-16': '⚡',
  'print-16': '🖨️',
  'question-mark-circle-16': '❓',
  'quicklink-16': '🔗',
  'minimize-16': '🗕',
  'minus-16': '➖',
  'minus-circle-16': '⊖',
  'minus-circle-filled-16': '➖',
  'mobile-16': '📱',
  'monitor-16': '🖥️',
  'moon-16': '🌙',
  'moon-down-16': '🌘',
  'moon-up-16': '🌔',
  'moonrise-16': '🌅',
  'mountain-16': '🏔️',
  'mouse-16': '🖱️',
  'move-16': '📦',
  'mug-16': '☕',
  'mug-steam-16': '☕',
  'multiply-16': '✖️',
  'music-16': '🎵',
  'network-16': '🌐',
  'new-document-16': '📄',
  'new-folder-16': '📁',
  'number-00-16': '0️⃣',
  'number-01-16': '1️⃣',
  'number-02-16': '2️⃣',
  'number-03-16': '3️⃣',
  'number-04-16': '4️⃣',
  'number-05-16': '5️⃣',
  'number-06-16': '6️⃣',
  'number-07-16': '7️⃣',
  'number-08-16': '8️⃣',
  'number-09-16': '9️⃣',
  'number-10-16': '🔟',
  'number-11-16': '1️⃣1️⃣',
  'number-12-16': '1️⃣2️⃣',
  'number-13-16': '1️⃣3️⃣',
  'number-14-16': '1️⃣4️⃣',
  'number-15-16': '1️⃣5️⃣',
  'number-16-16': '1️⃣6️⃣',
  'number-17-16': '1️⃣7️⃣',
  'number-18-16': '1️⃣8️⃣',
  'number-19-16': '1️⃣9️⃣',
  'number-20-16': '2️⃣0️⃣',
  'number-21-16': '2️⃣1️⃣',
  'number-22-16': '2️⃣2️⃣',
  'number-23-16': '2️⃣3️⃣',
  'number-24-16': '2️⃣4️⃣',
  'number-25-16': '2️⃣5️⃣',
  'number-26-16': '2️⃣6️⃣',
  'number-27-16': '2️⃣7️⃣',
  'number-28-16': '2️⃣8️⃣',
  'number-29-16': '2️⃣9️⃣',

  // Icons 401-450 that were missing (StarDisabled through TwoPeople)
  'star-disabled-16': '☆',
  'stars-16': '✨',
  'stop-16': '⏹️',
  'stop-filled-16': '⏹️',
  'stopwatch-16': '⏱️',
  'store-16': '🏪',
  'strike-through-16': '~~',
  'sun-16': '☀️',
  'sunrise-16': '🌅',
  'swatch-16': '🎨',
  'switch-16': '🔀',
  'syringe-16': '💉',
  'tack-16': '📌',
  'tack-disabled-16': '📌',
  'tag-16': '🏷️',
  'temperature-16': '🌡️',
  'tennis-ball-16': '🎾',
  'terminal-16': '💻',
  'text-16': '📝',
  'text-cursor-16': '📝',
  'text-input-16': '📝',
  'text-selection-16': '📝',
  'thumbs-down-16': '👎',
  'thumbs-down-filled-16': '👎',
  'thumbs-up-16': '👍',
  'thumbs-up-filled-16': '👍',
  'ticket-16': '🎫',
  'torch-16': '🔦',
  'train-16': '🚂',
  'trash-16': '🗑️',
  'tray-16': '📋',
  'tree-16': '🌳',
  'trophy-16': '🏆',
  'two-people-16': '👥',
  'underline-16': '📝',
  'undo-16': '↶',
  'upload-16': '⬆️',
  'uppercase-16': '🔤',
  'video-16': '📹',
  'video-disabled-16': '📹',
  'wallet-16': '👛',
  'wand-16': '🪄',
  'warning-16': '⚠️',
  'waveform-16': '〰️',
  'weights-16': '🏋️',
  'wifi-16': '📶',
  'wifi-disabled-16': '📶',
  'wind-16': '💨',
  'windsock-16': '🎏',
  'wrench-screwdriver-16': '🔧',
  'wrist-watch-16': '⌚',
  'x-mark-circle-16': '❌',
  'x-mark-circle-filled-16': '❌',
  'x-mark-circle-half-dash-16': '❌',
  'x-mark-top-right-square-16': '❌',
  'xmark-16': '❌',
  // Aliases (deprecated icons that point to existing ones)
  'blank-document-16': '📄',
  'gift-16': '🎁',
  'glasses-16': '👓',
  'globe-01-16': '🌍',
  'goal-16': '🥅',
  'hammer-16': '🔨',
  'hard-drive-16': '💾',
  'hashtag-16': '#️⃣',
  'heading-16': '📝',
  'headphones-16': '🎧',
  'heart-16': '❤️',
  'heart-disabled-16': '💔',
  'heartbeat-16': '💓',
  'highlight-16': '🖍️',
  'hourglass-16': '⏳',
  'house-16': '🏠',
  'humidity-16': '💧',
  'image-16': '🖼️',
  'important-01-16': '❗',
  'info-01-16': 'ℹ️',
  'italics-16': '𝐼',
  'key-16': '🔑',
  'keyboard-16': '⌨️',
  'layers-16': '📚',
  'leaderboard-16': '🏆',
  'leaf-16': '🍃',
  'light-bulb-16': '💡',
  'light-bulb-off-16': '💡',
  'line-chart-16': '📈',
  'link-16': '🔗',
  'livestream-01-16': '📹',
  'livestream-disabled-01-16': '📹',
  'lock-16': '🔒',
  'lock-disabled-16': '🔓',
  'lock-unlocked-16': '🔓',
  'logout-16': '🚪',
  'lorry-16': '🚚',
  'lowercase-16': '🔤',
  'magnifying-glass-16': '🔍',
  'male-16': '♂️',
  'map-16': '🗺️',
  'mask-16': '😷',
  'maximize-16': '🔍',
  'medical-support-16': '🏥',
  'megaphone-16': '📢',
  'computer-chip-16': '🔲',
  'memory-stick-16': '💾',
  'microphone-16': '🎤',
  'microphone-disabled-16': '🎤',
  
  // Missing icons from range 101-150 (Compass through Germ)
  'compass-16': '🧭',
  'contrast-16': '🔳',
  'cricket-ball-16': '🏏',
  'crop-16': '✂️',
  'crown-16': '👑',
  'crypto-16': '₿',
  'delete-document-16': '🗑️',
  'dna-16': '🧬',
  'dot-16': '⚫',
  'droplets-16': '💧',
  'duplicate-16': '📋',
  'edit-shape-16': '✏️',
  'eject-16': '⏏️',
  'ellipsis-16': '⋯',
  'ellipsis-vertical-16': '⋮',
  'emoji-16': '😀',
  'emoji-sad-16': '😢',
  'eraser-16': '🧹',
  'exclamationmark-16': '❗',
  'exclamationmark-2-16': '‼️',
  'exclamationmark-3-16': '❕',
  'eye-16': '👁️',
  'eye-dropper-16': '💧',
  'female-16': '♀️',
  'film-strip-16': '🎞️',
  'finder-16': '🔍',
  'fingerprint-16': '🖐️',
  'footprints-16': '👣',
  'forward-16': '⏭️',
  'forward-filled-16': '⏭️',
  'fountain-tip-16': '🖋️',
  'full-signal-16': '📶',
  'game-controller-16': '🎮',
  'gauge-16': '📊',
  'geopin-16': '📍',
  'germ-16': '🦠',
  
  'quotation-marks-16': '"',
  'quote-block-16': '💬',
  'racket-16': '🎾',
  'raindrop-16': '💧',
  'raycast-logo-neg-16': '⚡',
  'raycast-logo-pos-16': '⚡',
  'receipt-16': '🧾',
  'redo-16': '↪️',
  'remove-person-16': '👤',
  'repeat-16': '🔁',
  'replace-16': '🔄',
  'replace-one-16': '🔄',
  'reply-16': '↩️',
  'rewind-16': '⏪',
  'rewind-filled-16': '⏪',
  'rocket-16': '🚀',
  'rosette-16': '🏵️',
  'rotate-anti-clockwise-16': '🔄',
  'rotate-clockwise-16': '🔄',
  'rss-16': '📡',
  'ruler-16': '📏',
  'save-document-16': '💾',
  'shield-01-16': '🛡️',
  'short-paragraph-16': '📝',
  'shuffle-16': '🔀',
  'signal-0-16': '📶',
  'signal-1-16': '📶',
  'signal-2-16': '📶',
  'signal-3-16': '📶',
  'snippets-16': '📄',
  'snowflake-16': '❄️',
  'soccer-ball-16': '⚽',
  'speaker-16': '🔊',
  'speaker-down-16': '🔉',
  'speaker-high-16': '🔊',
  'speaker-low-16': '🔈',
  'speaker-off-16': '🔇',
  'speaker-on-16': '🔊',
  'speaker-up-16': '🔊',
  'speech-bubble-16': '💬',
  'speech-bubble-active-16': '💬',
  'speech-bubble-important-16': '❗',
  'square-ellipsis-16': '⋯',
  'stacked-bars-1-16': '📊',
  'stacked-bars-2-16': '📊',
  'stacked-bars-3-16': '📊',
  'stacked-bars-4-16': '📊',
  'star-16': '⭐',
  'star-circle-16': '⭐',
  'app-window-sidebar-right-16': '🪟'
}

// Helper function to get emoji for an icon
export function getIconEmoji(icon: RaycastIcon | string): string {
  const iconString = typeof icon === 'string' ? icon : icon
  return iconToEmoji[iconString] || '❓'
}

// Icon component that displays emoji
interface IconProps {
  source: RaycastIcon | string
}

export function Icon({ source }: IconProps): any {
  return getIconEmoji(source)
}