import React, { createContext, useContext, useState, useRef, useEffect, Children, isValidElement, cloneElement } from 'react'
import { useKeyboard } from '@opentui/react'
import { ActionPanel } from '@termcast/api/src/actions'
import { logger } from '@termcast/api/src/logger'
import { FormValues, FormProps, FormItemProps, FormEvent, FormEventType, FormItemRef, FormValue_2, FormValues_2, FormProps_2, FormItemProps_2 } from '@termcast/api/src/form/types'
...
export const Form: FormType = (props) => {
    ...
    useKeyboard((evt) => {
        if (evt.name === 'tab') {
            const currentIndex = focusedField ? fieldOrder.current.indexOf(focusedField) : -1
            let nextIndex: number
            
            if (evt.shift) {
                // Shift+Tab: go to previous field
                nextIndex = currentIndex > 0 ? currentIndex - 1 : fieldOrder.current.length - 1
            } else {
                // Tab: go to next field
                nextIndex = currentIndex < fieldOrder.current.length - 1 ? currentIndex + 1 : 0
            }
...
// These will be assigned after imports to avoid circular dependency
Form.TextField = null
