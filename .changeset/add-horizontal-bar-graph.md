...
Each row shows the category label on the left and a single horizontal stacked bar using the same default bar character as `Histogram`. The chart area grows to fill available space, while the legend only takes the width needed for colored series labels and percentages. Legend rows are sorted by highest percentage first. Chart components that use the shared default palette now start with orange, gray, cyan, purple, yellow, green, and blue for categorical data.
